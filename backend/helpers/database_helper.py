...
def if_user_exist(email: str):
    is_exist = db.query(models.Users).filter(
...
def add_new_user(user_schema):
    ...
    new_user = models.Users(c_id=user_schema.c_id, fullname=user_schema.fullname, email=user_schema.email, password=passhash.genrate_hash_password(password, salt).decode(
    ), salt=salt, contact_no=user_schema.contact_no, working_under=user_schema.working_under, dob=user_schema.dob, isactive=1, role_id=user_schema.role_id, created_at=datetime.datetime.now())
    db.add(new_user)
    db.commit()
    db.close()
...
def list_of_cid():
    companyid = db.query(models.Company).with_entities(
...
def add_new_company(data):
    new_company = models.Company(company_name=data.company_name, country=data.country, state=data.state, city=data.city, pincode=data.pincode,
                                 department=data.department, branch=data.branch, address=data.address, created_at=datetime.datetime.now(), isactive=1)
    db.add(new_company)
    db.commit()
    db.close()
